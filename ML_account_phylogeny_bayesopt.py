...
ML model training for BC using the best performing models form BenchmarkDR.
...
parser.add_argument('-g', '--groups', action='store', dest='groups',
                    help='''Value to select the phylogenetic groups for ML training (i.e., cc, mlst, kmer_95, poppunk)''')
...
path=os.path.join(outd,f'{method}_{group_l}_MLtraining.log')
...
for model, values in param_grid[method].items():
    ...
    # adjusting linear class cause it takes so long
    if method == 'classification' and model == 'sklearn_LR_l1':
        bayes_iter=10

    # Inner cross-validation for parameter search
    hyperopt = BayesSearchCV(tmp_model, tmp_params, random_state=3, n_iter=bayes_iter, cv=inner_cv, n_jobs=4, n_points=4, scoring=score_hyper)    

    print(f"Working on: {tmp_model}")

    
    # Outer cross-validation to compute the testing score
    cv_result = cross_validate(hyperopt, X, y, cv=outer_cv, groups=groups, fit_params={'groups': groups}, verbose=1, n_jobs=10, scoring=scores, return_train_score=True, return_estimator=True)
    print(
    ...
    if method == 'classification':
        best_score=abs(cv_result[f'test_{score_hyper}']).max()
